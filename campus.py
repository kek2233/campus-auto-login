...
def default_gateway():
    ...
    try:
        out = subprocess.run(["ipconfig"], capture_output=True, text=True,
                             encoding="gbk", errors="ignore", timeout=10, creationflags=0x08000000).stdout
        # 按适配器切段（段首行顶格且以冒号结尾）
        sections = []
        cur = []
...
def saved_profiles():
    try:
        out = subprocess.run(["netsh", "wlan", "show", "profiles"], capture_output=True,
                             text=True, encoding="gbk", errors="ignore", timeout=10, creationflags=0x08000000).stdout
...
def connect_wifi(ssid):
    ...
    log("正在连接 WiFi：「" + ssid + "」……")
    try:
        subprocess.run(["netsh", "wlan", "connect", "name=" + ssid], capture_output=True, timeout=15, creationflags=0x08000000)
    except Exception:
        pass
